...
pyv = glvars.pyv
pyv.bootstrap_e()
pygame = pyv.pygame  # use the legacy interface
...
def terminate():
    print('changing vars.gameover')
    glvars.pyv.vars.gameover = True

curr_mode = ''
input_finished = False
gl_cursor = None
gl_cursor_blink = None
prompt_str = None
fgcol, bgcol = None, None
cursorTimestamp = None
cursorShow = None  # contains what should be displayed as cursor, right now

input_x, input_y = None, None 

# The "pos" parameter can either be the string "left" (where the x, y coordinates refer to the top left corner of the text box)
# or "center" (where the x, y coordinates refer to the top center of the text box).
gl_pos = 'left'
MAX_LEN=12


def inputMode(prompt, fg_col, bg_col, allowed=None, cursor='_', cursorBlink=False):
    """Takes control of the program when called. This function displays a prompt on the screen (the "prompt" string)
    parameter) on the screenSurf surface at the input_x, input_y coordinates.
    The text is in the fgcol color with a bgcol color
    background. You can optionally specify maxlen for a maximum length of the user's response. "allowed" is a string
    of allowed characters (if the player can only type numbers, say) and ignores all other keystrokes.
    ...
    cursor."""
    global curr_mode, input_finished, gl_cursor, gl_cursor_blink, prompt_str, cursorTimestamp, cursorShow
    global fgcol, bgcol
    fgcol = fg_col
    bgcol = bg_col
    prompt_str = prompt
    cursorShow = gl_cursor = cursor
    
    gl_cursor_blink = cursorBlink
    inputText = ''
...
def checkForKeyPress():
    ...
    for event in pyv.evsys0.get():
        if event.type == pyv.evsys0.QUIT:
            terminate()
        if event.type == pyv.evsys0.KEYUP:
            if event.key == pyv.evsys0.K_ESCAPE: # pressing escape quits
                terminate()
            return event.key
    return False
...
def getDefaultSettings():#screenSurf):
    ...
    # while chr(key) != 'v' and chr(key) != 'p':
        # key = waitForPlayerToPressKey()
    # res = chr(key)  # returns 'v' or 'p'
    res = 'p'

    return p1name, p2name, points, gravity, res
...
def getShot(scr_ref, p1name, p2name, playerNum):
    """getShot() is called when we want to get the angle and velocity from the player."""
    global input_x, input_y

    pygame.draw.rect(scr_ref, SKY_COLOR, (0, 0, 200, 50))
    pygame.draw.rect(scr_ref, SKY_COLOR, (550, 0, 00, 50))

    drawText(p1name, scr_ref, 2, 2, WHITE_COLOR, SKY_COLOR)
    drawText(p2name, scr_ref, 538, 2, WHITE_COLOR, SKY_COLOR)
    ...
    wait_for = 'angle'
    input_x, input_y = x, 18
    inputMode('Angle:  ', WHITE_COLOR, SKY_COLOR, allowed='0123456789')
    ...
    # TODO: once the angle has been read (input_finished was set to True...)
    # then we will have to do the same input with velocity,
    # and once the velocity has been read,
    # we will be able to proceed to the rest of the computations
    # to do so, pass these informations to the rest of the program
    # # return (angle, velocity)

    # velocity = ''
    # while velocity == '':
        # velocity = inputMode('Velocity:  ', x, 34, WHITE_COLOR, SKY_COLOR, maxlen=3, allowed='0123456789')
    # if velocity is None:
        # terminate()
    # velocity = int(velocity)

    # <><> Erase the user's input
    #  Nota bene Tom : all the code below... Will probably get removed
    #
    # drawText('Angle:   ' + str(angle), scr_ref, x, 2, SKY_COLOR, SKY_COLOR)
    # drawText('Velocity:   ' + str(angle), scr_ref, x, 2, SKY_COLOR, SKY_COLOR)
    # pygame.display.update()
...
# make things global
GOR_DOWN_SURF = GOR_LEFT_SURF = GOR_RIGHT_SURF = BAN_RIGHT_SURF = BAN_LEFT_SURF = BAN_UP_SURF = BAN_DOWN_SURF = None
SUN_NORMAL_SURF = SUN_SHOCKED_SURF = STAR_SURF = None
sunRect = winSurface = None
...
def init(vmst=None):
    global GOR_DOWN_SURF, GOR_LEFT_SURF, GOR_RIGHT_SURF, BAN_RIGHT_SURF, BAN_LEFT_SURF, BAN_UP_SURF, BAN_DOWN_SURF
    global SUN_NORMAL_SURF, SUN_SHOCKED_SURF, STAR_SURF
    global screen, sunRect
...
def update(time_info=None):
    ...
    if curr_mode == 'input':
        ...
        if gl_cursor and gl_cursor_blink and time.time() - 1.0 > cursorTimestamp:
            if cursorShow == gl_cursor:
                cursorShow = '   '
            else:
                cursorShow = gl_cursor
            cursorTimestamp = time.time()

        for event in pyv.evsys0.get():
            if event.type == pyv.evsys0.QUIT:
                terminate()
            elif event.type == pyv.evsys0.KEYUP:
                if event.key == pyv.evsys0.K_ESCAPE:
                    return None
                elif event.key == pyv.evsys0.K_RETURN:
                    done = True
                    if cursorShow:
                        cursorShow = '   '
                elif event.key == pyv.evsys0.K_BACKSPACE:
                    if len(glvars.inputText):
                        #drawText(prompt_str + glvars.inputText + cursorShow, screen, textrect.left, textrect.top, bgcol, bgcol, 'left')
                        glvars.inputText = glvars.inputText[:-1]
                else:
                    if len(glvars.inputText) >= MAX_LEN:  # or (allowed is not None and chr(event.key) not in allowed):
                        continue
                    if event.key >= 32 and event.key < 128:
                        glvars.inputText += chr(event.key) #getModCase(chr(event.key), event.mod)

        textrect = drawText(prompt_str + cursorShow, screen, input_x, input_y, fgcol, bgcol, gl_pos)
        drawText(prompt_str + glvars.inputText + cursorShow, screen, textrect.left, textrect.top, fgcol, bgcol, 'left')
        pygame.display.update()
        GAME_CLOCK.tick(FPS)
    
        # ?? input_finished
        # angle = int(glvars.inputText) # angle

    else:
    
        # start a new game
        p1name, p2name, winPoints, gravity, nextScreen = getDefaultSettings()
        if nextScreen == 'v':
            showIntroScreen(screen, p1name, p2name)
        ...
        while p1score < winPoints and p2score < winPoints:
            ...
            # Do all the drawing.
            screen.blit(skylineSurf, (0,0))
            drawGorilla(screen, gorPos[0][0], gorPos[0][1], 0)
            drawGorilla(screen, gorPos[1][0], gorPos[1][1], 0)
            drawWind(screen, wind)
            drawSun(screen)
            displayScore(screen, p1score, p2score)

            pygame.display.update()

            # enter the input mode
            getShot(screen, p1name, p2name, turn)
            return
            
            # TODO proceed to the rest of the game computations
