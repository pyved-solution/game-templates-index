...
class IntroCompo(pyv.EvListener):
    ...
    def _update_playertypes(self):
        glvars.pltype1 = glvars.OMEGA_PL_TYPES[self.idx_pl1]
        glvars.pltype2 = glvars.OMEGA_PL_TYPES[self.idx_pl2]
        self.pltypes_labels[0].text = glvars.pltype1
        self.pltypes_labels[1].text = glvars.pltype2
    ...
    def __init__(self):
        ...
        # - v: buttons
        def rotatepl1():
            self.idx_pl1 = (self.idx_pl1 + 1) % len(glvars.OMEGA_PL_TYPES)
            self._update_playertypes()

        def rotatepl2():
            self.idx_pl2 = (self.idx_pl2 + 1) % len(glvars.OMEGA_PL_TYPES)
            self._update_playertypes()

        def rotleft_pl1():
            self.idx_pl1 = (self.idx_pl1 - 1)
            if self.idx_pl1 < 0:
                self.idx_pl1 = -1 + len(glvars.OMEGA_PL_TYPES)
            self._update_playertypes()

        def rotleft_pl2():
            self.idx_pl2 = (self.idx_pl2 - 1)
            if self.idx_pl2 < 0:
                self.idx_pl2 = -1 + len(glvars.OMEGA_PL_TYPES)
            self._update_playertypes()
