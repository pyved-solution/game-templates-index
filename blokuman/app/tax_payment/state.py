...
class TaxPaymentCtrl(kevent.EventReceiver):
...
class TaxPaymentView(kevent.EventReceiver):
    ...
    def __init__(self):
        ...
        pt_central = (scr_size[0] // 2, scr_size[1] // 2)
        ...
        temp = glvars.pygame.Rect((0, 0), self.BARRE_M_SIZE).inflate(self.PADDING_BARRE_M, self.PADDING_BARRE_M)
        self._fond_minage = glvars.pygame.Surface(temp.size).convert()
        ...
        self.etq_titre = self._hugefont.render(tsl(Labels.Minage1), False, self.TXT_COLOR_HL)
        ...
        self.etq_waitmsg = self._hugefont.render(tsl(Labels.Minage2), False, self.TXT_COLOR)
    ...
    def _maj_barre_minage(self, val):
        ...
        temp = glvars.pygame.Rect((0, 0), taille_barre)
        self._barre_minage = glvars.pygame.Surface(temp.size).convert()
...
class TaxPaymentState(kataen.BaseGameState):
