...
class Menu(ReceiverObj, Frect):  # N.B (tom) it would be better to inherit from EventReceiver +have a Frect attribute
    ...
    def __init__(self, dx, dy, w=300, h=100, anchor=ANCHOR_CENTER, menuitem=MENU_ITEM_COLOR,
        ...
        if font:
            ...
            self.font = pyv.vars.data['DejaVuSansCondensed-Bold']  # via engine
    ...
    def on_mousedown(self, ev):
        if ev.button == 1:
            mouse_pos = pyv.vscreen.proj_to_vscreen(pygame.mouse.get_pos())
    ...
    def on_mousemotion(self, ev):
        mouse_pos = pyv.vscreen.proj_to_vscreen(pygame.mouse.get_pos())  # still need to convert!
        moi = self.get_mouseover_item(mouse_pos)
        if moi is not None:
            self.set_item_by_position(moi)
...
class PopUpMenu(Menu):
    ...
    def __init__(self, w=200, h=250, predraw=None, border=default_border, **kwargs):
        mouse_pos = pyv.core.proj_to_vscreen(pygame.mouse.get_pos())
        x, y = mouse_pos
        x += 8
        y += 8
        sw, sh = pyv.vars.screen.get_size()
        if x + w + 32 > sw:
            x += -w - 32
        if y + h + 32 > sh:
            y += -h - 32
